import Link from "next/link";

export default function Footer() {
  return (
    <footer className="bg-[#1a1a1a] text-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          {/* Brand */}
          <div className="md:col-span-2">
            <h3 className="font-[family-name:var(--font-playfair)] text-2xl font-bold mb-2">
              Wright Lux Stays
            </h3>
            <div className="gold-line mb-4" />
            <p className="text-gray-400 text-sm leading-relaxed font-[family-name:var(--font-inter)] max-w-sm">
              Luxury vacation rentals curated with heart, in the places we love
              most — Sonoma, Kansas City, Austin & Carmel.
            </p>
            <div className="flex gap-4 mt-6">
              <a
                href="https://www.instagram.com/wrightluxstays/"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-full border border-[#C9A84C] flex items-center justify-center text-[#C9A84C] hover:bg-[#C9A84C] hover:text-white transition-colors"
                aria-label="Instagram"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
              <a
                href="https://www.facebook.com/profile.php?id=61553220984381"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 rounded-full border border-[#C9A84C] flex items-center justify-center text-[#C9A84C] hover:bg-[#C9A84C] hover:text-white transition-colors"
                aria-label="Facebook"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Locations */}
          <div>
            <h4 className="font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.15em] uppercase text-[#C9A84C] mb-4">
              Locations
            </h4>
            <ul className="space-y-2">
              {[
                { name: "Sonoma, CA", href: "/locations/sonoma" },
                { name: "Kansas City", href: "/locations/kansas-city" },
                { name: "Austin, TX", href: "/locations/austin" },
                { name: "Carmel, CA", href: "/locations/carmel" },
              ].map((l) => (
                <li key={l.href}>
                  <Link
                    href={l.href}
                    className="text-sm text-gray-400 hover:text-white transition-colors font-[family-name:var(--font-inter)]"
                  >
                    {l.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4 className="font-[family-name:var(--font-inter)] text-xs font-bold tracking-[0.15em] uppercase text-[#C9A84C] mb-4">
              Get In Touch
            </h4>
            <ul className="space-y-3">
              <li>
                <a
                  href="tel:+17075010199"
                  className="text-sm text-gray-400 hover:text-white transition-colors font-[family-name:var(--font-inter)]"
                >
                  (707) 501-0199
                </a>
              </li>
              <li>
                <a
                  href="mailto:amber@wrightluxstays.com"
                  className="text-sm text-gray-400 hover:text-white transition-colors font-[family-name:var(--font-inter)]"
                >
                  amber@wrightluxstays.com
                </a>
              </li>
              <li>
                <Link
                  href="/contact"
                  className="text-sm text-gray-400 hover:text-white transition-colors font-[family-name:var(--font-inter)]"
                >
                  Contact Form →
                </Link>
              </li>
              <li>
                <Link
                  href="/about"
                  className="text-sm text-gray-400 hover:text-white transition-colors font-[family-name:var(--font-inter)]"
                >
                  About Us →
                </Link>
              </li>
            </ul>
          </div>
        </div>

        <div className="mt-12 pt-8 border-t border-gray-800 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-gray-500 font-[family-name:var(--font-inter)]">
            © 2025 Wright Lux Stays. All rights reserved.
          </p>
          <p className="text-xs text-gray-600 font-[family-name:var(--font-inter)]">
            Channel management powered by{" "}
            <a
              href="https://www.guesty.com"
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#C9A84C] hover:text-white transition-colors"
            >
              Guesty
            </a>
          </p>
        </div>
      </div>
    </footer>
  );
}
